...
namespace gel
{
	namespace elf
	{
		...
		{
			return magic[0] == ELFMAG0 && magic[1] == ELFMAG1 && magic[2] == ELFMAG2 && magic[3] == ELFMAG3;
		}
		...
		{
			HashMap<elm::t::uint64, cstring> syms;
			SortedList<elm::t::uint64> _keys;

			for (auto k : *this)
				syms.add(k->value(), k->name());
			for (elm::t::uint64 ka : syms.keys())
				_keys.add(ka);

			int i = 0;
			for (elm::t::uint64 ka : _keys)
			{
				gel::Symbol *s = fetch(syms[ka]);
				if (s->size() == 0 && s->type() == gel::Symbol::FUNC && i < _keys.count() - 1)
				{
